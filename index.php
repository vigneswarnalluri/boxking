<!doctype html>
<html class="no-js" lang="zxx">

    <?php include './partials/head.php'?>

<body>
    <!--********************************
   	Code Start From Here 
	******************************** -->

    <?php include './partials/preloader.php'?>

    <!--==============================
    Search
    ==============================-->
    <?php include './partials/popup-box.php'?>

    <!--==============================
    SideMenu
    ==============================-->
    <?php include './partials/sidemenu.php'?>

    <!--==============================
    Mobile Menu & Header Area
    ============================== -->
    <?php include './partials/mobile-menu.php'?>
    <?php include './partials/header.php'?>

    <!--==============================
    Hero Area
    ==============================-->
    <div class="hero-wrapper hero-3" id="hero">
        <div class="hero-bg-3-1" data-mask-src="assets/img/hero/hero_bg_mask_3_1.png" data-bg-src="assets/img/hero_container.png"></div>
        <div class="hero-shadow-text">BOXKING</div>
        <div class="hero-bg-shape3-1 shape-mockup jump" data-bottom="3%" data-left="3%">
            <img src="assets/img/shape/sec-bg-shape2.png" alt="img">
        </div>
        
        <div class="container">
            <div class="hero-style3">
                <div class="row">
                    <div class="col-lg-12">
                        <span class="hero-subtitle">Rule Your Space. Build Your Kingdom. <img src="assets/img/icon/long-arrow-right2.svg" alt="img"></span>
                        <h1 class="hero-title">Premium Modular</h1>
                        <h1 class="hero-title style2">Building Solutions</h1>
                        <p class="hero-text">Custom Container Homes, Offices &amp; Modular Buildings Built with Strength, Precision and Innovation.</p>
                        <a href="contact.php" class="btn style2">GET FREE QUOTE <i class="ri-arrow-right-up-line"></i></a>
                    </div>
                </div>
            </div>                
        </div>
    </div>
    <!--======== / Hero Section ========-->

    <!--==============================
    About Area  
    ==============================-->
    <div class="about-area-3 space overflow-hidden" id="about">
        <div class="container">
            <div class="row gy-80 gx-100 align-items-end">                
                <div class="col-xl-7">
                    <div class="about-wrap3">
                        <div class="title-area mb-25">
                            <span class="sub-title text-theme">ABOUT US <i class="ri-arrow-right-down-line"></i></span>
                            <h2 class="sec-title">Built by Experts. Designed for Performance.</h2>
                            <p class="sec-text">BoxKing Modular Solutions combines the expertise of certified combination welders and master carpenters to create premium modular buildings that are durable, functional, and built to the highest standards. Every project is designed with precision, innovation, and craftsmanship to ensure exceptional quality.</p>
                            
                        </div>
                        <div class="checklist mb-35">
                            <ul>
                                <li><img src="assets/img/icon/about-checklsit-icon1-1.svg" alt="img">We exclusively use premium One-Trip shipping containers for maximum durability.
                                </li>
                                <li><img src="assets/img/icon/about-checklsit-icon1-1.svg" alt="img">Certified combination welding and master structural carpentry under one roof.</li>
                            </ul>
                        </div>
                        <div class="btn-wrap">
                            <div class="cta-grid-wrap">
                                <div class="icon-btn">
                                    <i class="ri-phone-fill"></i>
                                </div>
                                <div class="media-body">
                                    <a class="link" href="tel:+18005552695">+1 (800) 555-BOXKING</a>
                                    <h6 class="title">Need Help?</h6>
                                </div>
                            </div>
                            <div class="about-author-wrap">
                                <div class="author-thumb">
                                    <img src="assets/img/boxking-logo.png" alt="BoxKing" style="max-height: 45px; object-fit: contain;">
                                </div>
                                <div class="media-body">
                                    <div class="author-text">BoxKing Modular Solutions</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="about-thumb3 mt-70"> 
                        <div class="video-wrap about-img-1">
                            <img src="assets/img/about_modular.png" alt="Certified Modular Fabrication">
                        </div>
                    </div>                                  
                </div>
                <div class="col-xl-5 col-md-8">
                    <div class="about-thumb3 mb-30"> 
                        <div class="about-img-2" data-mask-src="assets/img/normal/about_mask3-2.png">
                            <img src="assets/img/container_office.png" alt="Container Office Build">
                        </div>
                    </div>
                    <p>Sales, fabrication, customization, leasing, and installation of modular buildings, portable cabins, container homes, and custom steel structures.</p>
                    <div class="btn-group mt-35">
                        <a href="about.php" class="btn">Learn More About Us <i class="ri-arrow-right-up-line"></i></a>
                    </div>
                </div>
            </div>
        </div>
    </div> 
    
    <!--==============================
    Why Choose Area 03  
    ==============================-->
    <div class="why-area-3 space-top overflow-hidden" id="services">
        <div class="why-sec-bg3-1" data-bg-src="assets/img/bg/why-bg3-1.png"></div>
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-lg-6">
                    <div class="title-area text-center">
                        <span class="sub-title text-theme">OUR SERVICES <i class="ri-arrow-right-down-line"></i></span>
                        <h2 class="sec-title">Comprehensive Modular &amp; Container Solutions</h2>
                    </div>
                </div>
            </div>
            <div class="row gy-50">
                <div class="col-xl-4">
                    <ul class="why-tab-wrap nav nav-pills" role=tablist>
                        <li class="nav-item">
                          <button class="nav-link active" id="why-pill-1-tab" data-bs-toggle="pill" data-bs-target="#why-pill-1" type="button" role="tab" aria-controls="why-pill-1" aria-selected="true">Container Offices <i class="ri-arrow-right-down-line"></i></button>
                        </li>
                        <li class="nav-item" role="presentation">
                          <button class="nav-link" id="why-pill-2-tab" data-bs-toggle="pill" data-bs-target="#why-pill-2" type="button" role="tab" aria-controls="why-pill-2" aria-selected="false">Container Homes <i class="ri-arrow-right-down-line"></i></button>
                        </li>
                        <li class="nav-item" role="presentation">
                          <button class="nav-link" id="why-pill-3-tab" data-bs-toggle="pill" data-bs-target="#why-pill-3" type="button" role="tab" aria-controls="why-pill-3" aria-selected="false">Portable Cabins <i class="ri-arrow-right-down-line"></i></button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="why-pill-4-tab" data-bs-toggle="pill" data-bs-target="#why-pill-4" type="button" role="tab" aria-controls="why-pill-4" aria-selected="false">Modular Buildings <i class="ri-arrow-right-down-line"></i></button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="why-pill-5-tab" data-bs-toggle="pill" data-bs-target="#why-pill-5" type="button" role="tab" aria-controls="why-pill-5" aria-selected="false">Custom Fabrication <i class="ri-arrow-right-down-line"></i></button>
                        </li>
                    </ul>
                </div>
                <div class="col-xl-8">
                    <div class="tab-content">
                        <div class="tab-pane fade show active" id="why-pill-1" role="tabpanel" aria-labelledby="why-pill-1-tab">
                            <div class="row gx-80 gy-40 align-items-center">
                                <div class="col-lg-5">
                                    <div class="why-thumb-wrap3-1">
                                        <div class="why-tab-thumb">
                                            <img src="assets/img/container_office.png" alt="Container Office">
                                        </div>
                                        <div class="why-text-wrap">
                                            <h4 class="title">Service</h4>
                                            <hr class="line">
                                            <div class="number">01</div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-7">
                                    <div class="why-content-wrap">
                                        <h4 class="title">About this Service</h4>
                                        <h5 class="subtitle">Container Offices</h5>
                                        <p class="text">Functional, climate-controlled, mobile office workspaces built from premium certified One-Trip containers. Fully equipped with electrical, HVAC, and modern insulation.</p>
                                        <div class="checklist mb-35">
                                            <ul>
                                                <li><img src="assets/img/icon/about-checklsit-icon1-1.svg" alt="img">Commercial Site Offices starting from $19,950
                                                </li>
                                            </ul>
                                        </div>
                                        <div class="btn-group mt-35">
                                            <a href="#contact" class="btn style3">Get Free Quote <i class="ri-arrow-right-up-line"></i></a>
                                        </div>
                                    </div>                                
                                </div>
                            </div>
                        </div>
                        <div class="tab-pane fade" id="why-pill-2" role="tabpanel" aria-labelledby="why-pill-2-tab">
                            <div class="row gx-80 gy-40 align-items-center">
                                <div class="col-lg-5">
                                    <div class="why-thumb-wrap3-1">
                                        <div class="why-tab-thumb">
                                            <img src="assets/img/container_home.png" alt="Container Home">
                                        </div>
                                        <div class="why-text-wrap">
                                            <h4 class="title">Service</h4>
                                            <hr class="line">
                                            <div class="number">02</div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-7">
                                    <div class="why-content-wrap">
                                        <h4 class="title">About this Service</h4>
                                        <h5 class="subtitle">Container Homes</h5>
                                        <p class="text">Eco-friendly, architecturally striking modular container residences designed for luxury living with master structural carpentry and modern interior finishes.</p>
                                        <div class="checklist mb-35">
                                            <ul>
                                                <li><img src="assets/img/icon/about-checklsit-icon1-1.svg" alt="img">Residential Container Homes starting from $29,950
                                                </li>
                                            </ul>
                                        </div>
                                        <div class="btn-group mt-35">
                                            <a href="#contact" class="btn style3">Get Free Quote <i class="ri-arrow-right-up-line"></i></a>
                                        </div>
                                    </div>                                
                                </div>
                            </div>
                        </div>
                        <div class="tab-pane fade" id="why-pill-3" role="tabpanel" aria-labelledby="why-pill-3-tab">
                            <div class="row gx-80 gy-40 align-items-center">
                                <div class="col-lg-5">
                                    <div class="why-thumb-wrap3-1">
                                        <div class="why-tab-thumb">
                                            <img src="assets/img/hero_container.png" alt="Portable Cabins">
                                        </div>
                                        <div class="why-text-wrap">
                                            <h4 class="title">Service</h4>
                                            <hr class="line">
                                            <div class="number">03</div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-7">
                                    <div class="why-content-wrap">
                                        <h4 class="title">About this Service</h4>
                                        <h5 class="subtitle">Portable Cabins</h5>
                                        <p class="text">Heavy-duty portable cabins ideal for remote job sites, security guard posts, emergency shelters, and workforce accommodation units.</p>
                                        <div class="checklist mb-35">
                                            <ul>
                                                <li><img src="assets/img/icon/about-checklsit-icon1-1.svg" alt="img">Rapid Deployment &amp; Turnkey Site Installation
                                                </li>
                                            </ul>
                                        </div>
                                        <div class="btn-group mt-35">
                                            <a href="#contact" class="btn style3">Get Free Quote <i class="ri-arrow-right-up-line"></i></a>
                                        </div>
                                    </div>                                
                                </div>
                            </div>
                        </div>
                        <div class="tab-pane fade" id="why-pill-4" role="tabpanel" aria-labelledby="why-pill-4-tab">
                            <div class="row gx-80 gy-40 align-items-center">
                                <div class="col-lg-5">
                                    <div class="why-thumb-wrap3-1">
                                        <div class="why-tab-thumb">
                                            <img src="assets/img/about_modular.png" alt="Modular Buildings">
                                        </div>
                                        <div class="why-text-wrap">
                                            <h4 class="title">Service</h4>
                                            <hr class="line">
                                            <div class="number">04</div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-7">
                                    <div class="why-content-wrap">
                                        <h4 class="title">About this Service</h4>
                                        <h5 class="subtitle">Modular Buildings</h5>
                                        <p class="text">Scalable multi-unit modular structures for commercial complexes, educational classrooms, healthcare facilities, and retail spaces.</p>
                                        <div class="checklist mb-35">
                                            <ul>
                                                <li><img src="assets/img/icon/about-checklsit-icon1-1.svg" alt="img">Scalable Off-Site Construction &amp; Modular Assembly
                                                </li>
                                            </ul>
                                        </div>
                                        <div class="btn-group mt-35">
                                            <a href="#contact" class="btn style3">Get Free Quote <i class="ri-arrow-right-up-line"></i></a>
                                        </div>
                                    </div>                                
                                </div>
                            </div>
                        </div>
                        <div class="tab-pane fade" id="why-pill-5" role="tabpanel" aria-labelledby="why-pill-5-tab">
                            <div class="row gx-80 gy-40 align-items-center">
                                <div class="col-lg-5">
                                    <div class="why-thumb-wrap3-1">
                                        <div class="why-tab-thumb">
                                            <img src="assets/img/diy_container_kit.png" alt="Custom Fabrication">
                                        </div>
                                        <div class="why-text-wrap">
                                            <h4 class="title">Service</h4>
                                            <hr class="line">
                                            <div class="number">05</div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-7">
                                    <div class="why-content-wrap">
                                        <h4 class="title">About this Service</h4>
                                        <h5 class="subtitle">Custom Fabrication &amp; DIY Kits</h5>
                                        <p class="text">Bespoke structural steel welding, container cutouts, custom windows/doors, and pre-engineered DIY container modification kits starting from $12,950.</p>
                                        <div class="checklist mb-35">
                                            <ul>
                                                <li><img src="assets/img/icon/about-checklsit-icon1-1.svg" alt="img">Certified Combination Welders &amp; Custom Steel Modifications
                                                </li>
                                            </ul>
                                        </div>
                                        <div class="btn-group mt-35">
                                            <a href="#contact" class="btn style3">Get Free Quote <i class="ri-arrow-right-up-line"></i></a>
                                        </div>
                                    </div>                                
                                </div>
                            </div>
                        </div>
                    </div>
                    
                </div>
                
            </div>
        </div>
    </div>

    <!--==============================
    Process Area  
    ==============================-->
    <section class="process-area-2 space overflow-hidden">
        <div class="container">
            <div class="row justify-content-between align-items-center gy-40">
                <div class="col-lg-6">
                    <div class="title-area text-md-start text-center">
                        <span class="sub-title text-theme">WHY CHOOSE US <i class="ri-arrow-right-down-line"></i></span>
                        <h2 class="sec-title">Built with Strength, Precision &amp; Innovation</h2>
                    </div>
                    <div class="process-thumb2-1">
                        <img src="assets/img/about_modular.png" alt="BoxKing Modular Quality">
                    </div>
                </div>
                <div class="col-lg-6">
                    <div class="process-grid-list style2">
                        <div class="process-grid-list-bg-text">
                            BOXKING
                        </div>
                        <div class="process-grid">
                            <div class="process-grid-number">01</div>
                            <div class="process-grid-details">
                                <h3 class="process-grid-title">Premium One-Trip Containers</h3>
                                <p class="process-grid-text">Exclusively using single-trip steel containers for maximum durability, structural rigidity, and zero rust.</p>
                            </div>
                        </div>
                        <div class="process-grid">
                            <div class="process-grid-number">02</div>
                            <div class="process-grid-details">
                                <h3 class="process-grid-title">Certified Combination Welding</h3>
                                <p class="process-grid-text">Master structural carpentry combined with certified heavy-duty steel welding expertise under one roof.</p>
                            </div>
                        </div>
                        <div class="process-grid">
                            <div class="process-grid-number">03</div>
                            <div class="process-grid-details">
                                <h3 class="process-grid-title">Custom Design &amp; Modern Interiors</h3>
                                <p class="process-grid-text">Tailored floor plans, luxury finishes, climate control, and full turnkey electrical and plumbing engineering.</p>
                            </div>
                        </div>
                        <div class="process-grid">
                            <div class="process-grid-number">04</div>
                            <div class="process-grid-details">
                                <h3 class="process-grid-title">Fast Turnaround &amp; Installation</h3>
                                <p class="process-grid-text">Off-site modular manufacturing delivers completed structures faster than traditional construction with nationwide delivery.</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            
        </div>
    </section> 

    <!--==============================
    Portfolio Area  
    ==============================-->
    <div class="portfolio-area-3 space-bottom" id="portfolio">
        <div class="container">
            <div class="row justify-content-between">
                <div class="col-lg-7">
                    <div class="title-area text-lg-start text-center">
                        <span class="sub-title text-theme">OUR PRODUCTS &amp; PROJECTS <i class="ri-arrow-right-down-line"></i></span>
                        <h2 class="sec-title">Explore Featured Models &amp; Pricing</h2>
                    </div>
                </div>
                <div class="col-lg-auto">
                    <div class="sec-btn">
                        <a href="#contact" class="btn">Request Custom Quote <i class="ri-arrow-right-up-line"></i></a>
                    </div>
                </div>
            </div>
            <div class="portfolio-slider3 overflow-hidden">
                <div class="row gy-30 gx-30 global-carousel" data-slide-show="2" data-ml-slide-show="2" data-lg-slide-show="1">
                    <div class="col-lg-6">
                        <div class="portfolio-card">
                            <div class="portfolio-card-thumb">
                                <img src="assets/img/diy_container_kit.png" alt="DIY Container Kits">
                                <a class="icon-btn popup-image" href="assets/img/diy_container_kit.png"><i class="ri-eye-line"></i></a>
                            </div>
                            <div class="portfolio-card-details">
                                <div class="media-left">
                                    <span class="portfolio-card-subtitle">Starting from $12,950</span>
                                    <h4 class="portfolio-card-title"><a href="#contact">DIY Container Kits</a></h4>
                                    <p class="portofolio-card-text">Pre-engineered container modification kits &amp; components</p>
                                </div>  
                            </div>
                            <a href="#contact" class="btn">Get Details <i class="ri-arrow-right-up-line"></i></a>                          
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="portfolio-card">
                            <div class="portfolio-card-thumb">
                                <img src="assets/img/container_office.png" alt="Container Office">
                                <a class="icon-btn popup-image" href="assets/img/container_office.png"><i class="ri-eye-line"></i></a>
                            </div>
                            <div class="portfolio-card-details">
                                <div class="media-left">
                                    <span class="portfolio-card-subtitle">Starting from $19,950</span>
                                    <h4 class="portfolio-card-title"><a href="#contact">Container Office</a></h4>
                                    <p class="portofolio-card-text">Turnkey site offices with full insulation &amp; HVAC</p>
                                </div>  
                            </div>
                            <a href="#contact" class="btn">Get Details <i class="ri-arrow-right-up-line"></i></a>                          
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="portfolio-card">
                            <div class="portfolio-card-thumb">
                                <img src="assets/img/container_home.png" alt="Container Home">
                                <a class="icon-btn popup-image" href="assets/img/container_home.png"><i class="ri-eye-line"></i></a>
                            </div>
                            <div class="portfolio-card-details">
                                <div class="media-left">
                                    <span class="portfolio-card-subtitle">Starting from $29,950</span>
                                    <h4 class="portfolio-card-title"><a href="#contact">Container Home</a></h4>
                                    <p class="portofolio-card-text">Modern luxury multi-unit residential container living</p>
                                </div>  
                            </div>
                            <a href="#contact" class="btn">Get Details <i class="ri-arrow-right-up-line"></i></a>                          
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="portfolio-card">
                            <div class="portfolio-card-thumb">
                                <img src="assets/img/hero_container.png" alt="Portable Cabins">
                                <a class="icon-btn popup-image" href="assets/img/hero_container.png"><i class="ri-eye-line"></i></a>
                            </div>
                            <div class="portfolio-card-details">
                                <div class="media-left">
                                    <span class="portfolio-card-subtitle">Custom Fabrication</span>
                                    <h4 class="portfolio-card-title"><a href="#contact">Portable Site Cabins</a></h4>
                                    <p class="portofolio-card-text">Heavy-duty remote site accommodation units</p>
                                </div>  
                            </div>
                            <a href="#contact" class="btn">Get Details <i class="ri-arrow-right-up-line"></i></a>                          
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!--==============================
    Counter Area 01  
    ==============================-->
    <div class="counter-area-1 space-bottom">
        <div class="container">
            <div class="row justify-content-between gy-40">
                <div class="col-auto">
                    <div class="counter-card">
                        <h2 class="counter-card_number"><span class="counter-number">26</span>k+</h2>
                        <p class="counter-card_text">Projects Completed</p>
                    </div>
                </div>
                <div class="col-auto">
                    <div class="counter-card">
                        <h2 class="counter-card_number"><span class="counter-number">98</span>%</h2>
                        <p class="counter-card_text">Customers Satisfied</p>
                    </div>
                </div>
                <div class="col-auto">
                    <div class="counter-card">
                        <h2 class="counter-card_number"><span class="counter-number">20</span>M</h2>
                        <p class="counter-card_text">Special Machinery</p>
                    </div>
                </div>
                <div class="col-auto">
                    <div class="counter-card">
                        <h2 class="counter-card_number"><span class="counter-number">30</span>+</h2>
                        <p class="counter-card_text">Years in Business</p>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!--==============================
    Contact Area  
    ==============================-->
    <section class="contact-area-2 space-bottom overflow-hidden" id="contact">        
        <div class="container">
            <div class="contact-wrap2 space overflow-hidden" data-bg-src="assets/img/bg/contact-bg3-1.png">
                <div class="section-animation-shape1-1 shape-mockup animation-infinite" data-top="0" data-left="0" data-bg-src="assets/img/shape/global-line-shape1.png">
                </div>
                <div class="row gy-60 justify-content-lg-end justify-content-center">
                    <div class="col-xl-7">
                        <div class="contact-form-wrap">
                            <div class="title-area">
                                <span class="sub-title text-theme">Get Free Quote <i class="ri-arrow-right-down-line"></i></span>
                                <h2 class="sec-title">Have a project in mind?</h2>
                            </div>
                            <form action="mail.php" method="POST" class="contact-form ajax-contact">
                                <div class="row gy-4">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <input type="text" class="form-control" name="name" id="name" placeholder="Your Name">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <input type="email" class="form-control" name="email" id="email" placeholder="Email Address">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <input type="tel" class="form-control" name="number" id="number" placeholder="Phone Number">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <select name="subject" id="subject" class="single-select nice-select form-select">
                                                <option value="" disabled selected hidden>Select Product / Service</option>
                                                <option value="Container Office">Container Office</option>
                                                <option value="Container Home">Container Home</option>
                                                <option value="Portable Cabin">Portable Cabin</option>
                                                <option value="Modular Building">Modular Building</option>
                                                <option value="Custom Fabrication">Custom Fabrication</option>
                                                <option value="DIY Container Kit">DIY Container Kit</option>
                                                <option value="Sales & Leasing">Sales &amp; Leasing Inquiry</option>
                                            </select>
                                        </div>
                                    </div>
                                    
                                    <div class="col-12">
                                        <div class="form-group ">
                                            <textarea name="message" id="message" cols="30" rows="3" class="form-control" placeholder="Message..."></textarea>
                                        </div>
                                    </div>
                                    <div class="form-btn col-12">
                                        <button class="btn w-100">Submit Now <i class="ri-arrow-right-up-line"></i></button>
                                    </div>
                                </div>
                                <p class="form-messages mb-0 mt-3"></p>
                            </form>
                        </div>
                    </div>            
                </div>
            </div>
            
        </div>
    </section>   

    <!--==============================
    Testimonial Area  
    ==============================-->
    <div class="testimonial-area-3 overflow-hidden">
        <div class="container">
            <div class="row justify-content-between">
                <div class="col-lg-7">
                    <div class="title-area text-lg-start text-center">
                        <span class="sub-title text-theme">Testimonials <i class="ri-arrow-right-down-line"></i></span>
                        <h2 class="sec-title">What client saying about us</h2>
                    </div>
                </div>
                <div class="col-lg-auto">
                    <div class="sec-btn btn-wrap">
                        <div class="client-group-thumb">
                            <img src="assets/img/normal/client_group_1-2.png" alt="img">
                        </div>
                        <div class="testi-counter-wrap">
                            <h3 class="testi-counter-number"><span class="counter-number">2</span>m+</h3>
                            <p class="testi-counter-text">Success Peoples</p>                            
                        </div>
                    </div>
                </div>
            </div>
            <div class="row global-carousel testi-slider3 slider-shadow" data-slide-show="3" data-lg-slide-show="2" data-dots="true" data-xl-dots="true" data-ml-dots="true" data-center-mode="true" data-xl-center-mode="true" data-ml-center-mode="true">
                <div class="col-lg-4">
                    <div class="testi-card style3">
                        <div class="testi-card-thumb">
                            <img src="assets/img/testimonial/testi_2_2.png" alt="img">
                            <div class="media-body">
                                <h4 class="testi-card_title">Best Construction!</h4>
                                <div class="testi-card_review">
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                </div>
                            </div>                                       
                        </div>  
                        <div class="testi-card_content">
                            <p class="testi-card_text">BoxKing delivered our custom container office on schedule with unbelievable build quality. Highly recommended master craftsmanship!</p>
                            <div class="testi-card-profile">
                                <h4 class="testi-profile-title">Joshua Pul</h4>
                                <span class="testi-profile-desig">/ CEO Industry</span>                                                                       
                            </div>
                        </div>                        
                        <div class="quote-icon">
                            <img src="assets/img/icon/quote3.svg" alt="img">
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="testi-card style3">
                        <div class="testi-card-thumb">
                            <img src="assets/img/testimonial/testi_2_3.png" alt="img">
                            <div class="media-body">
                                <h4 class="testi-card_title">Good Services!</h4>
                                <div class="testi-card_review">
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                </div>
                            </div>                                       
                        </div>  
                        <div class="testi-card_content">
                            <p class="testi-card_text">BoxKing delivered our custom container office on schedule with unbelievable build quality. Highly recommended master craftsmanship!</p>
                            <div class="testi-card-profile">
                                <h4 class="testi-profile-title">Aya Nikola</h4>
                                <span class="testi-profile-desig">/ Sr. Manager</span>                                                                       
                            </div>
                        </div>  
                        <div class="quote-icon">
                            <img src="assets/img/icon/quote3.svg" alt="img">
                        </div>                      
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="testi-card style3">
                        <div class="testi-card-thumb">
                            <img src="assets/img/testimonial/testi_2_1.png" alt="img">
                            <div class="media-body">
                                <h4 class="testi-card_title">Best Company!</h4>
                                <div class="testi-card_review">
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                </div>
                            </div>                                       
                        </div>  
                        <div class="testi-card_content">
                            <p class="testi-card_text">BoxKing delivered our custom container office on schedule with unbelievable build quality. Highly recommended master craftsmanship!</p>
                            <div class="testi-card-profile">
                                <h4 class="testi-profile-title">Christopher</h4>
                                <span class="testi-profile-desig">/ Engineer</span>                                                                       
                            </div>
                        </div>  
                        <div class="quote-icon">
                            <img src="assets/img/icon/quote3.svg" alt="img">
                        </div>                      
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="testi-card style3">
                        <div class="testi-card-thumb">
                            <img src="assets/img/testimonial/testi_2_2.png" alt="img">
                            <div class="media-body">
                                <h4 class="testi-card_title">Best Construction!</h4>
                                <div class="testi-card_review">
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                </div>
                            </div>                                       
                        </div>  
                        <div class="testi-card_content">
                            <p class="testi-card_text">BoxKing delivered our custom container office on schedule with unbelievable build quality. Highly recommended master craftsmanship!</p>
                            <div class="testi-card-profile">
                                <h4 class="testi-profile-title">Joshua Pul</h4>
                                <span class="testi-profile-desig">/ CEO Industry</span>                                                                       
                            </div>
                        </div>                        
                        <div class="quote-icon">
                            <img src="assets/img/icon/quote3.svg" alt="img">
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="testi-card style3">
                        <div class="testi-card-thumb">
                            <img src="assets/img/testimonial/testi_2_3.png" alt="img">
                            <div class="media-body">
                                <h4 class="testi-card_title">Good Services!</h4>
                                <div class="testi-card_review">
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                </div>
                            </div>                                       
                        </div>  
                        <div class="testi-card_content">
                            <p class="testi-card_text">BoxKing delivered our custom container office on schedule with unbelievable build quality. Highly recommended master craftsmanship!</p>
                            <div class="testi-card-profile">
                                <h4 class="testi-profile-title">Aya Nikola</h4>
                                <span class="testi-profile-desig">/ Sr. Manager</span>                                                                       
                            </div>
                        </div>  
                        <div class="quote-icon">
                            <img src="assets/img/icon/quote3.svg" alt="img">
                        </div>                      
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="testi-card style3">
                        <div class="testi-card-thumb">
                            <img src="assets/img/testimonial/testi_2_1.png" alt="img">
                            <div class="media-body">
                                <h4 class="testi-card_title">Best Company!</h4>
                                <div class="testi-card_review">
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                    <i class="ri-star-fill"></i>
                                </div>
                            </div>                                       
                        </div>  
                        <div class="testi-card_content">
                            <p class="testi-card_text">BoxKing delivered our custom container office on schedule with unbelievable build quality. Highly recommended master craftsmanship!</p>
                            <div class="testi-card-profile">
                                <h4 class="testi-profile-title">Christopher</h4>
                                <span class="testi-profile-desig">/ Engineer</span>                                                                       
                            </div>
                        </div>  
                        <div class="quote-icon">
                            <img src="assets/img/icon/quote3.svg" alt="img">
                        </div>                      
                    </div>
                </div>
            </div>         
        </div>
    </div>

    <!--==============================
    Client Area  
    ==============================-->
    <div class="client-area-2 text-center space overflow-hidden">
        <div class="container">
            <div class="row global-carousel client-slider2" data-slide-show="5" data-lg-slide-show="4" data-md-slide-show="3" data-sm-slide-show="2">
                <div class="col-lg-auto">
                    <div class="client-logo">
                        <a href="index.php"><img src="assets/img/client/client-2-1.svg" alt="img"></a>
                    </div>
                </div>
                <div class="col-lg-auto">
                    <div class="client-logo">
                        <a href="index.php"><img src="assets/img/client/client-2-2.svg" alt="img"></a>
                    </div>
                </div>
                <div class="col-lg-auto">
                    <div class="client-logo">
                        <a href="index.php"><img src="assets/img/client/client-2-3.svg" alt="img"></a>
                    </div>
                </div>
                <div class="col-lg-auto">
                    <div class="client-logo">
                        <a href="index.php"><img src="assets/img/client/client-2-4.svg" alt="img"></a>
                    </div>
                </div>
                <div class="col-lg-auto">
                    <div class="client-logo">
                        <a href="index.php"><img src="assets/img/client/client-2-5.svg" alt="img"></a>
                    </div>
                </div>
                <div class="col-lg-auto">
                    <div class="client-logo">
                        <a href="index.php"><img src="assets/img/client/client-2-1.svg" alt="img"></a>
                    </div>
                </div>
                <div class="col-lg-auto">
                    <div class="client-logo">
                        <a href="index.php"><img src="assets/img/client/client-2-2.svg" alt="img"></a>
                    </div>
                </div>
                <div class="col-lg-auto">
                    <div class="client-logo">
                        <a href="index.php"><img src="assets/img/client/client-2-3.svg" alt="img"></a>
                    </div>
                </div>
                <div class="col-lg-auto">
                    <div class="client-logo">
                        <a href="index.php"><img src="assets/img/client/client-2-4.svg" alt="img"></a>
                    </div>
                </div>
                <div class="col-lg-auto">
                    <div class="client-logo">
                        <a href="index.php"><img src="assets/img/client/client-2-5.svg" alt="img"></a>
                    </div>
                </div>
            </div>
        </div>
    </div> 

    <!--==============================
    Team Area  
    ==============================-->
    <div class="team-area-2 space-bottom">
        <div class="container">
            <div class="row justify-content-between align-items-center">
                <div class="col-lg-7 col-md-7">
                    <div class="title-area text-md-start text-center">
                        <span class="sub-title text-theme">Worker <i class="ri-arrow-right-down-line"></i></span>
                        <h2 class="sec-title">Meet with our expert team</h2>
                    </div>
                </div>
                <div class="col-md-auto">
                    <div class="sec-btn">
                        <a href="about.php" class="btn">View All Worker <i class="ri-arrow-right-up-line"></i></a>
                    </div>
                </div>
            </div>
            
            <div class="row gy-30 justify-content-center">
                <div class="col-lg-4 col-md-6">
                    <div class="team-card style2">                        
                        <div class="team-card_img">
                            <img src="assets/img/team/team-2-1.png" alt="img">
                            <div class="team-social_wrap">
                                <div class="social-btn">
                                    <a href="https://facebook.com/"><i class="ri-facebook-fill"></i></a>
                                    <a href="https://www.twitter.com/"><i class="ri-twitter-x-line"></i></a>
                                    <a href="https://instagram.com/"><i class="ri-instagram-line"></i></a>                           
                                </div>
                            </div>                     
                        </div>
                        <div class="team-card_content">
                            <h4 class="team-card_title"><a href="team-details.php">Richard Jackson</a>
                            </h4>
                            <span class="team-card_desig">CEO of Construz</span>
                            
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 col-md-6">
                    <div class="team-card style2">                        
                        <div class="team-card_img">
                            <img src="assets/img/team/team-2-2.png" alt="img">
                            <div class="team-social_wrap">
                                <div class="social-btn">
                                    <a href="https://facebook.com/"><i class="ri-facebook-fill"></i></a>
                                    <a href="https://www.twitter.com/"><i class="ri-twitter-x-line"></i></a>
                                    <a href="https://instagram.com/"><i class="ri-instagram-line"></i></a>                           
                                </div>
                            </div>                     
                        </div>
                        <div class="team-card_content">
                            <h4 class="team-card_title"><a href="team-details.php">Mark Ronaldo</a>
                            </h4>
                            <span class="team-card_desig">Engineer</span>
                            
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 col-md-6">
                    <div class="team-card style2">                        
                        <div class="team-card_img">
                            <img src="assets/img/team/team-2-3.png" alt="img">
                            <div class="team-social_wrap">
                                <div class="social-btn">
                                    <a href="https://facebook.com/"><i class="ri-facebook-fill"></i></a>
                                    <a href="https://www.twitter.com/"><i class="ri-twitter-x-line"></i></a>
                                    <a href="https://instagram.com/"><i class="ri-instagram-line"></i></a>                           
                                </div>
                            </div>                     
                        </div>
                        <div class="team-card_content">
                            <h4 class="team-card_title"><a href="team-details.php">John Maxwell</a>
                            </h4>
                            <span class="team-card_desig">John Maxwell</span>                            
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!--==============================
    CTA Area 03  
    ==============================-->
    <div class="cta-area-3">
        <div class="container">
            <div class="cta-wrap3 text-center" data-bg-src="assets/img/hero_container.png" data-overlay="title" data-opacity="6">
                <div class="title-area mb-30">
                    <span class="sub-title text-white">Rule Your Space. Build Your Kingdom. <i class="ri-arrow-right-down-line"></i></span>
                    <h2 class="cta-title style2">Your Custom Container</h2>
                    <h3 class="cta-title text-white">Starts Here</h3>
                </div>
                <div class="btn-wrap justify-content-center">
                    <a href="#contact" class="btn">Get Free Quote <i class="ri-arrow-right-up-line"></i></a>
                </div>
            </div>
        </div>
    </div>

    <!--==============================
    Blog Area  
    ==============================-->
    <section class="blog-area-2 space">
        <div class="section-animation-shape1-1 shape-mockup animation-infinite" data-top="0" data-bg-src="assets/img/shape/global-line-shape1.png">
        </div>
        <div class="container">
            <div class="row justify-content-lg-between justify-content-center align-items-center">
                <div class="col-lg-6">
                    <div class="title-area text-lg-start text-center">
                        <span class="sub-title text-theme">Daily update <i class="ri-arrow-right-down-line"></i></span>
                        <h2 class="sec-title">Our Latest News & Blog</h2>
                    </div>
                </div>
                <div class="col-md-auto">
                    <div class="sec-btn">
                        <a href="blog.php" class="btn">View All News <i class="ri-arrow-right-up-line"></i></a>
                    </div>
                </div>
            </div>
            <div class="row global-carousel blog-slider slider-shadow" data-slide-show="3" data-lg-slide-show="2" data-md-slide-show="2" data-sm-slide-show="1" data-xs-slide-show="1" data-dots="false">
                <div class="col-md-6 col-lg-4">
                    <div class="blog-card style2">
                        <div class="blog-img">
                            <img src="assets/img/blog/blog_1_1.png" alt="blog image">
                            <div class="blog-date">
                                <a href="blog.php"><span>17</span>JUN</a>
                                <div class="year">2024</div>
                            </div>
                        </div>
                        <div class="blog-content">
                            <div class="blog-meta">
                                <a href="blog.php">By Rebecca</a>
                                <a href="blog.php">Construction</a>
                            </div>
                            <h3 class="blog-title"><a href="blog-details.php">How to hire a contractor home renovation service</a></h3>
                            <p class="blog-text">Tortor posuere ac ut consequat. Tellusi elem isis etum sag ittis vitae et leo duis ut diam odio ut sem</p>
                            <a href="blog-details.php" class="btn style2">More Details <i class="ri-arrow-right-up-line"></i></a>
                        </div>
                    </div>
                </div>

                <div class="col-md-6 col-lg-4">
                    <div class="blog-card style2">
                        <div class="blog-img">
                            <img src="assets/img/blog/blog_1_2.png" alt="blog image">
                            <div class="blog-date">
                                <a href="blog.php"><span>17</span>JUN</a>
                                <div class="year">2024</div>
                            </div>
                        </div>
                        <div class="blog-content">
                            <div class="blog-meta">
                                <a href="blog.php">By Rebecca</a>
                                <a href="blog.php">Construction</a>
                            </div>
                            <h3 class="blog-title"><a href="blog-details.php">Started to develop a specific testing programs</a></h3>
                            <p class="blog-text">Tortor posuere ac ut consequat. Tellusi elem isis etum sag ittis vitae et leo duis ut diam odio ut sem</p>
                            <a href="blog-details.php" class="btn style2">More Details <i class="ri-arrow-right-up-line"></i></a>
                        </div>
                    </div>
                </div>

                <div class="col-md-6 col-lg-4">
                    <div class="blog-card style2">
                        <div class="blog-img">
                            <img src="assets/img/blog/blog_1_3.png" alt="blog image">
                            <div class="blog-date">
                                <a href="blog.php"><span>17</span>JUN</a>
                                <div class="year">2024</div>
                            </div>
                        </div>
                        <div class="blog-content">
                            <div class="blog-meta">
                                <a href="blog.php">By Rebecca</a>
                                <a href="blog.php">Construction</a>
                            </div>
                            <h3 class="blog-title"><a href="blog-details.php">How to stay motivated until a project is finished</a></h3>
                            <p class="blog-text">Tortor posuere ac ut consequat. Tellusi elem isis etum sag ittis vitae et leo duis ut diam odio ut sem</p>
                            <a href="blog-details.php" class="btn style2">More Details <i class="ri-arrow-right-up-line"></i></a>
                        </div>
                    </div>
                </div>

                <div class="col-md-6 col-lg-4">
                    <div class="blog-card style2">
                        <div class="blog-img">
                            <img src="assets/img/blog/blog_1_1.png" alt="blog image">
                            <div class="blog-date">
                                <a href="blog.php"><span>17</span>JUN</a>
                                <div class="year">2024</div>
                            </div>
                        </div>
                        <div class="blog-content">
                            <div class="blog-meta">
                                <a href="blog.php">By Rebecca</a>
                                <a href="blog.php">Construction</a>
                            </div>
                            <h3 class="blog-title"><a href="blog-details.php">How to hire a contractor home renovation service</a></h3>
                            <p class="blog-text">Tortor posuere ac ut consequat. Tellusi elem isis etum sag ittis vitae et leo duis ut diam odio ut sem</p>
                            <a href="blog-details.php" class="btn style2">More Details <i class="ri-arrow-right-up-line"></i></a>
                        </div>
                    </div>
                </div>

                <div class="col-md-6 col-lg-4">
                    <div class="blog-card style2">
                        <div class="blog-img">
                            <img src="assets/img/blog/blog_1_2.png" alt="blog image">
                            <div class="blog-date">
                                <a href="blog.php"><span>17</span>JUN</a>
                                <div class="year">2024</div>
                            </div>
                        </div>
                        <div class="blog-content">
                            <div class="blog-meta">
                                <a href="blog.php">By Rebecca</a>
                                <a href="blog.php">Construction</a>
                            </div>
                            <h3 class="blog-title"><a href="blog-details.php">Started to develop a specific testing programs</a></h3>
                            <p class="blog-text">Tortor posuere ac ut consequat. Tellusi elem isis etum sag ittis vitae et leo duis ut diam odio ut sem</p>
                            <a href="blog-details.php" class="btn style2">More Details <i class="ri-arrow-right-up-line"></i></a>
                        </div>
                    </div>
                </div>

                <div class="col-md-6 col-lg-4">
                    <div class="blog-card style2">
                        <div class="blog-img">
                            <img src="assets/img/blog/blog_1_3.png" alt="blog image">
                            <div class="blog-date">
                                <a href="blog.php"><span>17</span>JUN</a>
                                <div class="year">2024</div>
                            </div>
                        </div>
                        <div class="blog-content">
                            <div class="blog-meta">
                                <a href="blog.php">By Rebecca</a>
                                <a href="blog.php">Construction</a>
                            </div>
                            <h3 class="blog-title"><a href="blog-details.php">How to stay motivated until a project is finished</a></h3>
                            <p class="blog-text">Tortor posuere ac ut consequat. Tellusi elem isis etum sag ittis vitae et leo duis ut diam odio ut sem</p>
                            <a href="blog-details.php" class="btn style2">More Details <i class="ri-arrow-right-up-line"></i></a>
                        </div>
                    </div>
                </div>
                
            </div>
        </div>
    </section>   

    <!--==============================
        Footer Area
    ==============================-->
    <?php include './partials/footer.php'?>

    <!--********************************
	Code End  Here 
	******************************** -->

    <!-- Scroll To Top -->
    <?php include './partials/scroll-top.php'?>

    <!--==============================
    All Js File
    ============================== -->
    <?php include './partials/script.php'?>

</body>

</html>